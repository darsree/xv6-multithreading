...
sched(void)
{
  ...
  // VIZ FIX: log the exact tick p stops running, not just when it
  // started. record_schedstat() in scheduler() only ever logs a
  // dispatch (state==RUNNING); tools/viz had to GUESS each bar's end
  // by looking at the next record on the same cpu, capped by an
  // arbitrary MAX_BAR_TICKS. This second call gives an exact stop
  // timestamp — p->state here is never RUNNING (it was just set to
  // RUNNABLE/SLEEPING/ZOMBIE above), so plot_timeline.py can tell
  // "start" and "stop" records apart for free and draw exact bars,
  // with real (not guessed) idle gaps in between.
  record_schedstat(p, cpuid());
